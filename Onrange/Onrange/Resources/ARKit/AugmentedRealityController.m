...
- (void)updateLocations
{
    ...
    for (ARGeoCoordinate *item in [self coordinates]) {
        ...
        if ([self shouldDisplayCoordinate:item]) {
            ...
            int radialDistanceInt = roundf([item radialDistance]);
            int randomRadial = roundf(sin(sqrt(radialDistanceInt))*10*(-1));
//            NSLog(@"radial int: %d", radialDistanceInt);
//            NSLog(@"seno raiz: %f", roundf(sin(sqrt(radialDistanceInt))*10*(-1)));
            
            [markerView setFrame:CGRectMake(loc.x - width / 2.0, (loc.y + 140)-([item radialDistance]/200)-(randomRadial*randomRadial)/0.35, width, height)];
