...
- (void)carregaUsuarios {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@checkin/listaUsuariosCheckin/%d/%@/%d",ambienteAPI,(int)id_local,genero,(int)usuarioDispositivo.id_usuario]];
    ...
    [operation setCompletionBlockWithSuccess:^(RKObjectRequestOperation *operation, RKMappingResult *mappingResult) {
        ...
        [self.headerView.btnCheckinLocal setHidden:NO];
...
//    http://graph.facebook.com/873898955974353/picture?type=large
    ...
    [cell.imgFotoUsuario sd_setImageWithURL:[NSURL URLWithString:strURL] placeholderImage:[UIImage imageNamed:@"fb-medal2.png"] options:SDWebImageRefreshCached];
    
    if (usuario.liked == 1 && usuario.matched == 0) {
        cell.imgEasterEgg.hidden = YES;
        ...
        if([usuario.facebook_usuario isEqualToString:@"873898955974353"] || [usuario.facebook_usuario isEqualToString:@"10203049470773190"]){
            cell.imgEasterEgg.hidden = NO;
            cell.viewContainerUsuarios.backgroundColor = [UIColor colorWithHexString:@"#F46122"];
        }
    }
    ...
    if (kind == UICollectionElementKindSectionHeader) {
        ...
        UIImage *imgCheckin = [UIImage imageNamed:@"btn_checkin_quadrado"];
            ...
            if (self.usuarioEstaNoLocal == YES) {
                ...
                [self.headerView.btnCheckinLocal setImage:imgCheckout forState: UIControlStateNormal];
            }else{
                [self.headerView.btnCheckinLocal setImage:imgCheckin forState: UIControlStateNormal];
            }
        ...
        [self.headerView.lblNomeLocal setText:nome_local];
        [self.headerView.lblQtPessoas setText:nomeLocalCheckins];
...
- (IBAction)btCheckinLocal:(id)sender {
    ...
    if (self.usuarioEstaNoLocal == YES){
        UIActionSheet* actionCheckout = [[UIActionSheet alloc]
                                         initWithTitle:[NSString stringWithFormat:@"Ao sair do local, não será possível interagir com os usuários e suas curtidas serão apagadas. Confirma checkout em %@?", nome_local]
        ...
        UIActionSheet* actionCheckin = [[UIActionSheet alloc]
                                        initWithTitle:[NSString stringWithFormat:@"Se você tiver checkin em outro local, ele será apagado ao fazer um novo checkin. Confirma checkin em %@?", nome_local]
                                        delegate:(id<UIActionSheetDelegate>)self
                                        cancelButtonTitle:@"Sim"
                                        destructiveButtonTitle:@"Não"
                                        otherButtonTitles:nil];
