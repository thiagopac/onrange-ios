...
- (void)viewDidLoad
{
    ...
    MKCoordinateRegion adjustedRegion = [self.mapLocal regionThatFits:MKCoordinateRegionMakeWithDistance(startCoord, zoom, zoom)];
...
- (void)scrollViewDidScroll:(UIScrollView *)scrollView {
    ...
    if (y<0) {
        ...
        //
        MKCoordinateRegion region = MKCoordinateRegionMakeWithDistance (newCenter, zoom+(5*ypos), zoom+(5*ypos));
        [_mapLocal setRegion:region animated:NO];
        //
    }
}
...
- (void)carregaUsuarios {
    ...
    NSURL *url = [NSURL URLWithString:[NSString stringWithFormat:@"%@checkin/listaUsuariosCheckin/%d/MF/%d",ambienteAPI,(int)id_local,(int)usuarioDispositivo.id_usuario]];
